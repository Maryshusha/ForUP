...
public class functionsController implements Initializable {
    ...
    @FXML
    private TableColumn<getProductsData, String> add_product_col_composition;

    @FXML
    private TableView<getProductsData> add_product_tableview;

    @FXML
    private TextArea add_products_composition;
    ...
    public <T> void addSearchListener(TableView<T> table, TextField searchField, ObservableList<T> data) {
    ...
    public ObservableList<getProductsData> addProductsListData(){
        ...
        String sql = "select id_product, name_product, quantity_product, name_manufacturer, name_provider, name_catalog, price, image, composition from products p, manufacturers m, providers p2, catalogs c where p.id_manufacturer = m.id_manufacturer and p.id_provider = p2.id_provider and p.id_catalog = c.id_catalog";
    ...
    public void addProductsShowListData() {
        ...
        add_product_col_composition.setCellValueFactory(new PropertyValueFactory<>("productsComposition"));
    ...
    public void addProductSelect(){
        ...
        add_products_composition.setText(getProductsD.getProductsComposition());
    ...
    public void addProductsAdd() {
        ...
        String productComposition = add_products_composition.getText();
        ...
        if (productName.isEmpty() || productQuantityText.isEmpty() || productPriceText.isEmpty() ||
                manufacturerName.isEmpty() || providerName.isEmpty() || catalogName.isEmpty() || productComposition.isEmpty()) {
            return;
        }
        ...
        String sqlProduct = "INSERT INTO products (id_product, name_product, quantity_product, id_manufacturer, id_provider, id_catalog, price, image, composition) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        insertIntoDatabase(sqlProduct, productId, productName, productQuantity, manufacturerId, providerId, catalogId, productPrice, productImage, productComposition);
    ...
    public void addProductUpdate() {
        ...
        if (result.get() == ButtonType.OK){
            ...
            String sqlProduct = "UPDATE products SET name_product = ?, quantity_product = ?, id_manufacturer = ?, id_provider = ?, id_catalog = ?, price = ?, image = ?, composition = ? WHERE id_product = ?";
            insertIntoDatabase(sqlProduct, productName, productQuantity, manufacturerId, providerId, catalogId, productPrice, productImage, productComposition, productId);
    ...
    public void addProductsReset(){
        ...
        add_products_composition.setText("");
    ...
    public void switchForm(ActionEvent event){
        ...
        buttonStyles.put(home_btn, "-fx-background-color:  transparent; -fx-border-color:  #D9D2D0; -fx-border-radius: 20;");
        buttonStyles.put(add_products_btn, "-fx-background-color:  transparent; -fx-border-color:  #D9D2D0; -fx-border-radius: 20;");
        buttonStyles.put(order_btn, "-fx-background-color:  transparent; -fx-border-color:  #D9D2D0; -fx-border-radius: 20;");
        ...
        for (Node button : buttonStyles.keySet()) {
            ...
            if (button == source) {
                style += " -fx-background-radius: 19; -fx-border-color:  #D9D2D0; -fx-border-radius: 19; -fx-border-width: 2";
            }
        ...
        if (source == add_products_btn) {
            addProductsShowListData();
            addSearchListener(add_product_tableview, add_product_search, add_product_tableview.getItems());
        } else if (source == order_btn) {
            ordersShowListData();
            addOrderStatusData();
            addSearchListener(orders_tableview, orders_search, orders_tableview.getItems());
